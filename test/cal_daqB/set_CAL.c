...
//  char filename[256];
  ...
  if ((access("setting/setup_com.txt", 0)) == 0) {
    ...
    fscanf(fp_setup, "%ld", &cw);
    fscanf(fp_setup, "%ld", &run_number);
    fscanf(fp_setup, "%ld", &pedestal_trigger_interval);
    fscanf(fp_setup, "%ld", &trigger_enable);
    fscanf(fp_setup, "%ld", &multiplicity_thr);
    fscanf(fp_setup, "%ld", &trigger_delay);
    fscanf(fp_setup, "%ld", &thr);    
    fclose(fp_setup);
  }
...
//externcw unsigned long CALTCBread_TRIGGER_DELAY(int sid);
