...
int dbg_waveform(void)
{
  ...
  fp = fopen("/usr/local/notice/programmer/cal_daq/calLUT_1.dat", "rb");
  for (i = 0; i < ch_to_plot; i++)
    fread(offset, 1, 1024, fp);
  fread(offset, 1, 1024, fp);
  fclose(fp);
  ...
  for (evt = 0; evt < nevt; evt++) {
    ...
    plot->Reset();
    for (i = 0; i < 1023; i++) {
      plot->Fill(i, adc[i * 32 + ch_to_plot] + (offset[i] & 0xFF));
