...
int main(int argv, char** argc) {
  ...
  Option up_lut = Option("lut", 1, &dev, uploadLUT_action);

  Option* children[] = {&up_lut, &up_fx3, &up_fpga, &info, &set_a};
  dev.chidrensize = 5;
...
void uploadLUT_action(Option* option, void *data) {
  ...
  if (file.is_open()) {
    ...
    buffer = new char[length];
    file.read(buffer, length);
    cout << "Uploading calibration table. Please wait.\n";
    dev->uploadLUT(length, buffer);
    if (dev->getError() == SUCCESS) {
      cout << "Calibration table has been uploaded successfully.\n";
    } else
      cout << "Failed to upload calibration table.\n";
    delete[] buffer;
  } else {
    cout << "Could not open file: " << option->args[0] << endl;
  }
...
void help_action(Option* option, void *data) {
  cout << "NOTICE Korea (c) 2012 <http://www.noticekorea.com/>\n"
    ...
    "                             Option -dev has to be set.\n"
    "  -lut <filename>            :Upload calibration table. \n"
    "                             Option -dev has to be set.\n"
...
void Device::uploadFPGAFirmware(size_t size, char *data, char *filename) 
{
  ...
  if (rem)
    ncount = nsector * 256 + naddr + 2;
  else
    ncount = nsector * 256 + naddr + 1;
  count = 0;
...
void Device::uploadLUT(size_t size, char *data) 
{
  int nsector;
  int naddr;
  int rem;
  ...
  if (rem)
    ...
    ncount = nsector * 256 + naddr;
  count = 0;

  for (sector = 0; sector < nsector; sector++) {
    // erase sector
    ep0buffer[0] = (sector + 15) & 0xFF;
    error = write(VENDOR_FLASH_ERASE, 0, 0, 1, ep0buffer);
  ...
  ep0buffer[0] = 0;
  error = write(VENDOR_WRITE_CAL_FLAG, 0, 0, 1, ep0buffer);
